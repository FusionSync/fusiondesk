...
class LocalMacFileTransferSource final
    : public TransferSource,
      public ITransferFileContentProvider
{
public:
    LocalMacFileTransferSource(TransferSourceId sourceId,
        ...
        const TransferFileRangeRequest& request) override
    {
        ...
        if (request.sourceId != 0 && request.sourceId != sourceId_) {
            ...
            result.message = "macOS file source id is not found";
            return result;
        }
        if (request.fileIndex >= paths_.size() ||
            request.fileIndex >= fileList_.files.size()) {
            result.status = protocol::ResponseStatus::NotFound;
            result.message = "macOS file index is not found";
            return result;
        }
        ...
        if (request.objectId != descriptor.objectId) {
            result.status = protocol::ResponseStatus::NotFound;
            result.message = "macOS file object is not found";
            return result;
        }
        ...
        if (request.offset >
            ...
            result.message = "macOS file range offset is invalid";
            return result;
        }
        ...
        if (!file.is_open()) {
            result.status = protocol::ResponseStatus::NotFound;
            result.message = "macOS file cannot be opened";
            return result;
        }
        ...
        if (readBytes < 0) {
            ...
            result.message = "macOS file read failed";
            return result;
        }
...
bool readMacLocalFileList(TransferFileList& output,
                          std::vector<std::filesystem::path>* paths,
                          ...
                          const MacClipboardLocalFileOptions& options)
{
    ...
    if (paths != nullptr)
        paths->clear();
    if (options.maxFileCount == 0 || inputPaths.empty())
        return false;

    for (const std::filesystem::path& path : inputPaths) {
...
std::shared_ptr<TransferSource> createLocalMacFileTransferSource(
    ...
    std::uint64_t maxSingleFileBytes)
{
    return std::make_shared<LocalMacFileTransferSource>(
        sourceId,
        std::move(descriptor),
        std::move(fileList),
        std::move(paths),
