...
package_name="fusiondesk-$PLATFORM-$safe_version"
...
client_exe="$(find_exe fusiondesk_pc_client)"
agent_exe="$(find_exe fusiondesk_pc_agent)"
profile_plan_exe="$(find_exe fusiondesk_pc_profile_plan)"

cp "$client_exe" "$stage_root/bin/"
cp "$agent_exe" "$stage_root/bin/"
...
copy_qt_dependencies "$stage_root/bin/fusiondesk_pc_client"
copy_qt_dependencies "$stage_root/bin/fusiondesk_pc_agent"
copy_qt_dependencies "$stage_root/bin/fusiondesk_pc_profile_plan"

cat > "$stage_root/fusiondesk_pc_client.sh" <<'EOF'
#!/usr/bin/env bash
APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export LD_LIBRARY_PATH="$APP_DIR/lib:${LD_LIBRARY_PATH:-}"
exec "$APP_DIR/bin/fusiondesk_pc_client" "$@"
EOF

cat > "$stage_root/fusiondesk_pc_agent.sh" <<'EOF'
#!/usr/bin/env bash
APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export LD_LIBRARY_PATH="$APP_DIR/lib:${LD_LIBRARY_PATH:-}"
exec "$APP_DIR/bin/fusiondesk_pc_agent" "$@"
EOF

chmod +x "$stage_root/fusiondesk_pc_client.sh" "$stage_root/fusiondesk_pc_agent.sh"
...
- fusiondesk_pc_agent.sh
- fusiondesk_pc_client.sh

Smoke:
  ./fusiondesk_pc_agent.sh --smoke
  ./fusiondesk_pc_client.sh --smoke

Generate paired local TCP profiles:
  bin/fusiondesk_pc_profile_plan --client-profile samples/client.json --agent-profile samples/agent.json --channel control=127.0.0.1:47101 --channel small_data=127.0.0.1:47102 --channel main_screen=127.0.0.1:47103

This package is the FusionDesk engineering release zip. It is not a distro package.
EOF
...
Example:
  ../bin/fusiondesk_pc_profile_plan --client-profile client.json --agent-profile agent.json --channel control=127.0.0.1:47101 --channel small_data=127.0.0.1:47102 --channel main_screen=127.0.0.1:47103
EOF

if [[ "$SKIP_SMOKE" != "1" ]]; then
  "$stage_root/fusiondesk_pc_agent.sh" --smoke
  "$stage_root/fusiondesk_pc_client.sh" --smoke
fi
